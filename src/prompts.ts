...
export function buildContextPrompt(active: ActiveWorkflow): string {
	...
	const innerTotal =
		state.currentPath.length > 1
			? isSubworkflowRef(active.currentPhaseEntry)
	...
	if (nextPhase) {
		nextPhaseName = isSubworkflowRef(nextPhase)
	...
	const vars: Record<string, string> = {
		...
		previousPhaseName: getPreviousPhaseName(
			definition,
			innerSegment.phaseIndex,
		),
		nextPhaseName,
		blockedToolsList:
			blockedTools.length > 0 ? blockedTools.join(", ") : "(none)",
	...
	const lines: string[] = [];
	lines.push(
		`[Workflow path: ${breadcrumbNames} ▸ ${currentPhase.emoji} ${currentPhase.name}]`,
	);
	lines.push("");
	...
	lines.push("");
	lines.push(
		`**Available subagent profiles for this phase:** ${profiles.join(", ") || "(none)"}`,
	);
...
function getPreviousPhaseName(
	definition: { phases: PhaseEntry[] },
	currentIndex: number,
): string {
	if (currentIndex <= 0) return "(start)";
	const prev = definition.phases[currentIndex - 1];
	if (isSubworkflowRef(prev)) return prev.resolved.name ?? prev.workflowKey;
	return prev.name;
}

...
| `types.ts`             | Type definitions and discriminated-union guards                                                                                                   | `PhaseToolConfig`, `PathSegment`, `PhaseDefinition`, `SubworkflowReference`, `PhaseEntry`, `WorkflowDefinition`, `WorkflowState`, `ActiveWorkflow`, `HookStateMutation`, `GetState`, `SetState`, `GetDefinitions`, `ReloadDefinitions`, `isSubworkflowRef()`, `isPhaseDefinition()`, `normalizeWorkflowKey()`, `lookupWorkflowKey()` | —                                                                  |
...
| `config/validation.ts` | Workflow definition validation and subworkflow cycle detection (iterative DFS)                                                                    | `validateWorkflowDefinition()`, `detectCycles()`, `VALID_COMMAND_NAME_RE`, `CycleError` (type)                                                                                                                                                                                      | `types.ts`                                                         |
...
| `prompts.ts`           | Context prompt construction and default message templates                                                                                         | `buildContextPrompt()`, `flattenAllPhases()`, `DEFAULT_NOT_DONE_REMINDER`, `DEFAULT_COMPLETION_MESSAGE`, `DEFAULT_CANCELLED_MESSAGE`                                                                                                                                                                      | `types.ts`, `config/`, `state.ts`                                  |
...
| `node:path` | `join`, `resolve`, `relative`, `isAbsolute` — path construction, relative path computation, and path-safety checks                      |
